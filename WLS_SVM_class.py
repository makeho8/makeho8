...
class WLS_SVM(BaseEstimator):
    ...
    def fit(self, X, y):
        ...
        if self.kernel == None:
            ...
            I = np.identity(m+1)
            Y = (self.c1/self.c2)*I - (self.c1/self.c2)*GB.T.dot(np.linalg.inv((self.c2/self.c1)*IB + GB.dot(GB.T))).dot(GB)
            Z = (self.c3/self.c4)*I - (self.c3/self.c4)*HA.T.dot(np.linalg.inv((self.c4/self.c3)*IA + HA.dot(HA.T))).dot(HA)
        # class A
        self.WA = []
        self.bA = []
        for i in range(self.k):
